...
Resources can define custom row actions (e.g. activate/deactivate) that are shown in the grid. Pass an `actions` array on the resource in `sync_schema`:
      ...
      actions: [
        {
          ...
          actionExpression: \"data.status == 'Active' ? '/deactivate' : '/activate'\",
          labelExpression: \"data.status == 'Active' ? 'Deactivate' : 'Activate'\",
          confirm: true
        }
      ]
    }
...
* `permission` - the action button is only rendered when the user has this permission (superusers see all actions)
* `actionExpression` - evaluated per row with the record available as `data`; the result is used as the request path appended to `<endpoint>/<record.id>` (e.g. `/users/5/deactivate`)
* `labelExpression` - evaluated per row with the record available as `data`; the result is the button label. When it evaluates to a falsy value, the button is hidden for that row
* `confirm: true` - show a confirmation dialog before executing the action

The action is executed with a POST request and the grid refreshes afterwards. The resource must have an `endpoint` for actions to work (resources generated by `sync_schema` always have one).

The backend endpoint for a custom action is a regular Symfony route added to the resource controller with `write_file`:
...
final class UserController extends ResourceController
{
    ...
    {
        $this->denyAccessUnlessGranted('users.deactivate');

        $record = $this->entityManager->getRepository($this->getEntityClass())->find($id);
        if (!$record) {
...
12. **Use resource `actions` for row operations like activate/deactivate instead of custom pages**
